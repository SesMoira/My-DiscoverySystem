...
public class AccountTypeControllerTest {
    ...
    public void getAll() throws Exception{
        ...
        accountTypes.add(new AccountTypeDto("PLAY", "Play account type", LocalDate.parse("2020-04-01")));

        when(fetchAccountTypeFlow.getAllAccountTypes().thenReturn(accountTypes));
        MvcResult mvcResult = mockMvc.perform(get(String.format("")))
                .servletPath(APP_URL)
    ...
    public void create() throws Exception{
        ...
        String expectedResponse = "";
        AccountTypeDto accountType = AccountTypeDto("Miles", "Miles account type", LocalDate.parse("2020-01-01"));

        when(CreateAccountTypeFlow.create(eq(accountType))).then(returnsFirstArgs());
    ...
    public void deleteAccountType() throws Exception{
        String expectedResponse = "";
        AccountTypeDto accountType = AccountTypeDto("PLAY", "Play account type", LocalDate.parse("2020-01-01"));

        when(modifyAccountTypeFlow.deleteAccountType(anyString())).thenReturn(accountType);
        MvcResult mvcResult = mockMvc.perform(delete((String.format("")))
                            .servletPath(APP_URL)
                            .accept(MediaType.APPLICATION_JSON)
                             .contentType(MediaType.APPLICATION_JSON)
                .andExpect(status().isOk())
    ...
    public void updateAccountType() throws Exception{
        ...
        MvcResult mvcResult = mockMvc.perform(put((String.format("")))
                        .param("newAccountTypeName","New Play account type name")
        ...
        verify(modifyAccountTypeFlow, times(1)).updateAccountType(eq("PLAY"),
                eq("new play account type name"), eq(LocalDate.parse("2021-04-04")));
        assertEquals(expectedResponse, mvcResult.getResponse().getContentAsString());
    ...
    public void updateAccountTypeWithNoOptionalDate() throws Exception{
        ...
        MvcResult mvcResult = mockMvc.perform(put((String.format("")))
                ...
                .andReturn();
        verify(modifyAccountTypeFlow, times(1)).updateAccountType(eq("PLAY"),
                eq("new play account type name"), eq(null));
        assertEquals(expectedResponse, mvcResult.getResponse().getContentAsString());
